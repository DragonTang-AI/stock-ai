...
/**
 * 搜索股票（按名称或代码）
 * 后端路由: GET /api/v1/market/search?q=xxx&limit=20
 */
export async function searchStocks(q: string, limit = 10): Promise<SearchResult[]> {
  if (!q || !q.trim()) return []
  const response = await request<any>('/market/search', {
    method: 'GET',
